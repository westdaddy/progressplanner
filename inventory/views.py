...
def _render_filtered_products(
    ...
    group_balance_notes: list[dict[str, Any]] = []
    if group_inventory_totals or sales_group_totals:
        scope_label = selected_type_label or selected_style_label
        for group_name in sorted(
            set(group_inventory_totals.keys()) | set(sales_group_totals.keys()),
            key=str.lower,
        ):
            inv_total = group_inventory_totals.get(group_name, 0)
            sales_total = sales_group_totals.get(group_name, 0)
            balance_pct: Optional[float] = None

            if sales_total > 0:
                balance_pct = ((inv_total - sales_total) / sales_total) * 100
                if inv_total == sales_total:
                    message = "Stock level is in line with last year's sales."
                elif inv_total > sales_total:
                    message = (
                        f"Overstocked by {balance_pct:.0f}% compared to last year's sales."
                    )
                else:
                    message = (
                        f"Understocked by {balance_pct:.0f}% compared to last year's sales."
                    )
            else:
                qualifier = f" in {scope_label}" if scope_label else ""
                message = (
                    f"Stock on hand{qualifier} but no comparable sales data."
                )

            group_balance_notes.append(
                {
                    "label": group_name,
    ...
    context.update(
        {
            ...
            "group_balance_notes": group_balance_notes,
