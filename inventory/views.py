...
from django.db.models import Count, F, Q, Sum, Subquery, OuterRef, Value, IntegerField, DecimalField, Prefetch, FloatField, ExpressionWrapper, Avg
...
def home(request):
    ...
    for sale in sales_last_month.select_related('variant'):
        if sale.variant:
            cat = _simplify_type(sale.variant.type)
    ...
    for i in range(12):
        ...
        rev_last = (
            Sale.objects
                .filter(date__gte=last_year_month, date__lt=last_year_month + relativedelta(months=1))
                .aggregate(total=Sum('sold_value'))['total']
            or 0
        )
    ...
    latest_cost_qs = OrderItem.objects.filter(
        product_variant=OuterRef('pk'),
        date_arrived__isnull=False
    ).order_by('-date_arrived').values('item_cost_price')[:1]

    avg_cost_qs = OrderItem.objects.filter(
        product_variant=OuterRef('pk')
    ...
    variants = ProductVariant.objects.annotate(
        latest_inventory=Coalesce(
            Subquery(
                InventorySnapshot.objects
                    .filter(product_variant=OuterRef('pk'))
                    .order_by('-date')
        ...
        unit_cost=Coalesce(
            ...
            ExpressionWrapper(
                F('product__retail_price') * Value(Decimal('0.5')),       # 3) half of retail price
    ...
    inventory_value = variants.aggregate(
        total=Sum(
            ExpressionWrapper(
    ...
    on_order_value = incoming.aggregate(
        total=Sum(
            ExpressionWrapper(
    ...
    est_sales_value = calculate_estimated_inventory_sales_value(
        variants,
        _simplify_type  # pass your bucket-func in
    )
...
def product_list(request):
    ...
    products_qs = Product.objects.all().prefetch_related(
        Prefetch('variants', queryset=variants_qs, to_attr='variants_with_inventory')
    ).annotate(
    ...
    for product in products:
        ...
        product.variants_with_inventory.sort(
            key=lambda v: SIZE_ORDER.get(v.size, 9999)
        )
        ...
        if last_item:
            ...
            cost = order_items.aggregate(
                total_cost=Coalesce(
                    Sum(ExpressionWrapper(F('item_cost_price') * F('quantity'),
                                          output_field=DecimalField())),
                    Decimal('0.00')
                )
...
def product_detail(request, product_id):
    ...
    snaps = (
        InventorySnapshot.objects
        .filter(product_variant__product=product, date__gte=twelve_months_ago)
        .order_by('date')
    )
    ...
    speeds = {
        row['variant_code']: row['avg_speed']
        for row in safe_stock['safe_stock_data']
    }
...
def order_list(request):
    ...
    for i in range(4):
        ...
        incoming_qs = OrderItem.objects.filter(
            date_expected__gte=month_start,
            date_expected__lt=month_end
...
def inventory_snapshots(request):
    ...
    if selected_type != 'all':
        snap_qs  = snap_qs.filter(product_variant__type=selected_type)
        sale_qs  = sale_qs.filter(variant__type=selected_type)
        order_qs = order_qs.filter(product_variant__type=selected_type)
    ...
    total_sold = (
        sale_qs
        .filter(date__gte=six_mo_ago)
        .aggregate(total=Sum('sold_quantity'))['total']
        or 0
    )
    ...
    dim        = calendar.monthrange(last_snapshot_date.year,
                                     last_snapshot_date.month)[1]
    next1      = (last_snapshot_date.replace(day=1)
    ...
    forecast_data = [
        {"x": last_snapshot_date.isoformat(),
         "y": round(last_inventory)}
    ]
    ...
    avg_monthly_by_size = {
        row['size']: row['total_sold'] / 6
        for row in sales_by_size
    }
    ...
    stock_on_last = (
        snap_qs
        .filter(date=last_snapshot_date)
    ...
    color_map = {
        'XS': '#ef9a9a', 'S': '#f48fb1', 'M': '#ce93d8',
        'L': '#90caf9', 'XL': '#80deea', 'XXL': '#b0bec5'
    }

    # ——— after computing avg_monthly_by_size, scores, etc. —————
    size_order = ['XS', 'S', 'M', 'L', 'XL', 'XXL']

    size_mix = calculate_size_order_mix(
    ...
    # ——— 5) Render template —————————————————————————————————————
    return render(request, "inventory/inventory_snapshots.html", {
        "categories":    categories,
...
def returns(request):
    ...
    for i in range(12):
        ...
        returns_total = (
            Sale.objects
                .filter(date__gte=month_start, date__lt=next_month)
                .aggregate(total=Sum('return_value'))['total']
            or 0
        )
        ...
        Sale.objects
            ...
            .annotate(
                return_rate = ExpressionWrapper(
                    F('total_returned') * 1.0 / F('total_sold'),
    ...
    context = {
        # existing context
        'returns': Sale.objects
                       .filter(return_quantity__gt=0)
