...
from django.db.models import Count, F, Q, Sum, Subquery, OuterRef, Value, IntegerField, DecimalField, Prefetch, ExpressionWrapper
...
def calculate_size_order_mix(
    *,
    ...
    if category and category != 'all':
        sale_qs = sale_qs.filter(variant__type=category)
        snap_qs = snap_qs.filter(product_variant__type=category)
    ...
    ideal_pct = {size: (score / total_score) * 100
                 for size, score in demand_scores.items()}
    ...
    for sz in size_order:
        result.append({
            ...
            'sell_through': round(
                1.0 if (weighted_sales.get(sz, 0) + ending_stock_map.get(sz, 0)) == 0
...
def compute_sales_aggregates(product):
    ...
    avg_sale_price = round(sale_agg['total_val']/sale_agg['total_qty'], 2) if sale_agg['total_qty'] else 0

    # Ensure retail_price is numeric
    from decimal import Decimal
    retail_price = product.retail_price if product.retail_price is not None else Decimal('0.00')

    # Calculate discount percentage safely
    if retail_price > 0:
        discount_pct = round((retail_price - Decimal(avg_sale_price)) / retail_price * 100, 0)
    else:
        discount_pct = 0
...
def calculate_estimated_inventory_sales_value(variants, _simplify_type):
    ...
    stats = (
        Sale.objects
            .values('variant')
            .annotate(
                total_revenue=Sum('sold_value'),
                total_qty=Sum('sold_quantity')
            )
    )
    avg_price_map = {
        s['variant']: s['total_revenue'] / s['total_qty']
        for s in stats if s['total_qty']
    }
    ...
    category_avg = {
        ...
        for s in cat_stats if s['total_qty']
    }
    ...
    for v in variants:
        ...
        if unit_avg is None:
            unit_avg = category_avg.get(_simplify_type(v.type), Decimal('0'))
...
def compute_inventory_health_scores(variants, _simplify_type):
    ...
    sold_qs = (
        Sale.objects
            .filter(variant__in=variants, date__gte=six_months_ago)
            .values('variant')
    ...
    for v in variants:
        cat_counts[_simplify_type(v.type or '')] += 1
    ...
    for v in variants:
        ...
        last_arrival = (
            OrderItem.objects
                     .filter(product_variant=v, date_arrived__isnull=False)
                     .order_by('-date_arrived')
                     .values_list('date_arrived', flat=True)
                     .first()
        )
        if last_arrival:
            months_since = (today.year - last_arrival.year) * 12 + (today.month - last_arrival.month)
        else:
            months_since = 13
        if months_since < 6:
            restock_score = 2
...
def get_product_health_metrics(variants, _simplify_type):
    ...
    if total_inv:
        overall = sum(raw_scores.get(v.id, 0) * v.latest_inventory for v in variants) / total_inv
    else:
    ...
    def calc_sales_subscore(v):
        ...
        sold   = Sale.objects.filter(
            variant=v, date__gte=one_year_ago
        ).aggregate(q=Sum('sold_quantity'))['q'] or 0
        # category avg:
        cat_qs = Sale.objects.filter(
            variant__type=v.type, date__gte=one_year_ago
        ).aggregate(
            tot_rev=Sum('sold_value'),
            tot_qty=Sum('sold_quantity')
        )
        avg_sold = (cat_qs['tot_rev'] / cat_qs['tot_qty']) if cat_qs['tot_qty'] else 1
        rel_str = sold / avg_sold if avg_sold else 0
        if rel_str >= 1.2: return 3
        if rel_str >= 0.8: return 2
        return 1
    ...
    # 5) time in stock
    first_arrival = OrderItem.objects.filter(
        product_variant__in=variants,
        date_arrived__isnull=False
    ).order_by('date_arrived').first()
...
def calculate_dynamic_product_score(variants, simplify_type):
    ...
    sold_qs = (
        Sale.objects
            .filter(variant__in=variants, date__gte=six_mo_ago)
            .values('variant')
            .annotate(qty=Sum('sold_quantity'))
    )
    ...
    percent_remaining = (
        ...
        if total_available > 0 else Decimal('1')
    )

    has_core = any(
        v.latest_inventory > 0 and
        v.size in CORE_SIZES.get(simplify_type(v.type or ''), [])
        for v in variants
    )
...
def compute_product_health(product, variants, simplify_type):
    ...
    sold_qs = Sale.objects.filter(
        variant__in=variants, date__gte=three_mo_ago
    ).values('variant').annotate(q=Sum('sold_quantity'))
    ...
    sv      = normalize(avg_daily,
                        best=2*cat_avg_daily.get(simplify_type(variants[0].type),1),
                        worst=0)

    # --- 2. SELL-THROUGH RATE (last 6 mo) ---
    six_mo_ago = today - relativedelta(months=6)
    sold6   = Sale.objects.filter(
        variant__in=variants, date__gte=six_mo_ago
    ).aggregate(s=Sum('sold_quantity'))['s'] or 0
    ...
    core_sizes = CORE_SIZES.get(simplify_type(variants[0].type), [])
    core_in_stock = sum(
        1 for v in variants
        if v.size in core_sizes and v.latest_inventory>0
    )
    ...
    restocks = OrderItem.objects.filter(
        product_variant__in=variants,
        date_arrived__gte=year_ago
    ).values('date_arrived').distinct().count()
    rs_sc    = normalize(restocks, best=12, worst=0)

    # --- 7. RETURN RATE (last 6 mo) ---
    returned = Sale.objects.filter(
        variant__in=variants, date__gte=six_mo_ago
    ).aggregate(r=Sum('return_quantity'))['r'] or 0
